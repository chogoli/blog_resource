...
download_path="/root/auto-upload/downloads"
upload_path="/root/auto-upload/upload/OneDriveUploader"
...
Install_upload(){
    wget https://raw.githubusercontent.com/MoeClub/OneList/master/OneDriveUploader/amd64/linux/OneDriveUploader -P /root/auto-upload/upload/
    chmod +x /root/auto-upload/upload/OneDriveUploader
    ln -s /root/auto-upload/upload/OneDriveUploader /usr/local/bin/OneDriveUploader
    clear
    echo -e "${Green_font_prefix}选择你的OneDriver账号类型${Font_color_suffix} 
...
"
    ...
    [ -z "$num" ] && num=1
    echo -e "${Green_font_prefix}获取授权地址请复制下列对应的链接到浏览器打开登录后复制地址栏地址${Font_color_suffix}
${Green_font_prefix}国际版、个人版(家庭版):${Font_color_suffix}https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=78d4dc35-7e46-42c6-9023-2d39314433a5&response_type=code&redirect_uri=http://localhost/onedrive-login&response_mode=query&scope=offline_access%20User.Read%20Files.ReadWrite.All

${Green_font_prefix}中国版(世纪互联):${Font_color_suffix}https://login.chinacloudapi.cn/common/oauth2/v2.0/authorize?client_id=dfe36e60-6133-48cf-869f-4d15b8354769&response_type=code&redirect_uri=http://localhost/onedrive-login&response_mode=query&scope=offline_access%20User.Read%20Files.ReadWrite.All
"
    read -p "请输入你的OneDrive授权地址: " ODURL
    [ -z "$ODURL" ] && exit
    case "$num" in
	1)
	ODV="-a"
    ...
    *)
	clear
	echo -e "${Red_font_prefix}请输入正确数字 [1-3]${Font_color_suffix}"
	sleep 3s
	Menu
	;;
    esac
    OneDriveUploader $ODV \"$ODURL\"
    [ $? == 0 ] && echo -e "${Green_font_prefix}OneDriveUploader安装成功${Font_color_suffix}" && Upload_down_config || echo -e "${Red_font_prefix}OneDriveUploader安装失败${Font_color_suffix}"
    mv /root/.aria2c/auth.json /root/auto-upload/upload/
}
Install_aria2() {
    check_root
...
Uninstall_aria2() {
    ...
    if [[ ${unyn} == [Yy] ]]; then
        ...
        rm -rf /root/auto-upload && rm -rf /usr/local/bin/OneDriveUploader
        rm -rf "/etc/init.d/aria2"
        echo && echo "Aria2 + upload 卸载完成 !" && echo
    else
...
echo && echo -e " Aria2 一键安装管理脚本 增强版 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix} by \033[1;35mP3TERX.COM\033[0m
 ...
 ———————————————————————
 ${Green_font_prefix} 1.${Font_color_suffix} 安装 Aria2 + Upload
 ${Green_font_prefix} 2.${Font_color_suffix} 更新 Aria2
 ${Green_font_prefix} 3.${Font_color_suffix} 卸载 Aria2 + Upload
 ———————————————————————
